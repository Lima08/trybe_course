...
@patch('bowling_game.frame.randint')
def test_tenth_frame_play(randint_mock):
    randint_mock.return_value = 1
    tenth_frame = TenthFrame()
    tenth_frame.play()

    assert tenth_frame.first_roll == 1
    assert tenth_frame.second_roll == 1
    assert tenth_frame.third_roll == 1
    assert tenth_frame.type == FrameTypes.REGULAR
